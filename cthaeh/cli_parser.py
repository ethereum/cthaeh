...
from cthaeh.commands import do_initialize_database, do_main
...
database_url_parser.add_argument(
    ...
    type=str,
    dest="database_url",
    help=("The database url for the databse that should be used."),
)
database_url_parser.add_argument(
    "--db-memory",
    const="sqlite:///:memory:",
    action="store_const",
    dest="database_url",
    help=("Use an in-memory sqlite3 database."),
)

initialize_database_parser = subparser.add_parser(
    "init-db", help="Initialize the database schema"
)
...
loading_parser.add_argument(
    ...
    dest="concurrency",
    default=3,
    help=("The level of concurrency which which to pull data"),
)
loading_parser.add_argument(
    "--start-block",
    type=int,
    dest="start_block",
    help=("The starting block number to load"),
)
loading_parser.add_argument(
    "--end-block",
    dest="end_block",
...
jsonrpc_parser.add_argument(
    "--ipc-path",
    type=pathlib.Path,
    help=("The path for the IPC socket that will be used for the JSON-RPC server"),
)
jsonrpc_parser.add_argument(
    "--disable-jsonrpc", action="store_true", help=("Disable the JSON-RPC server")
)
